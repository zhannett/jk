...
function calculateGrandTotal() {
	...
	for (j = 1, max1 = $('.new_record').length; j < max1; j += 1) {
		...
		benAmountRem = parseFloat($('#amount2').val().replace(/,/g, '')) - grandTotal;$('#amount1').text('test')
...
$('#btnAddLine').click(function () {
	'use strict';
	var now = new Date(),
		amount1 = parseFloat($('#amount1').val().replace(/,/g, '')),
		amount2 = parseFloat($('#amount2').val().replace(/,/g, '')),
	...
	$('#grandtotalCur2').text($('#selectCurrency2 option:selected').val());
	...
	$('<li></li>')
		...
		.appendTo($newRecord)
		.text($('#selectCurrency2 option:selected').text());
	$('<li></li>')
			...
			$('#btnSaveEdited')
				...
				.click(function () {
					...
					if (editNum === 1 || editNum === 3 || editNum === 6 || editNum === 10 || editNum === 15 || editNum === 21 || editNum === 28 || editNum === 36 || editNum === 45 || editNum === 55) {
						($that).parentNode.childNodes[4].innerHTML = $('#benAmount').val();
						($that).parentNode.childNodes[6].innerHTML = $('#benProduct option:selected').val();
						($that).parentNode.childNodes[7].innerHTML = $('#selectBeneficiary option:selected').val();
						($that).parentNode.childNodes[12].innerHTML = ($that).parentNode.childNodes[4].innerHTML;
						($that).parentNode.childNodes[16].innerHTML = $('#beneficiaryAccountNum option:selected').val(); //beneficiaryAccountNum
						($that).parentNode.childNodes[17].innerHTML = $('#ffc option:selected').val(); //ffc
						($that).parentNode.childNodes[19].innerHTML = $('#newBenNotes').val(); //newBenNotes
						($that).parentNode.childNodes[20].innerHTML = $('#notesForBendix').val(); //notesForBendix
						calculateGrandTotal();
					}
				});
		});
	...
	$('<li></li>')
		...
		.click(function () {
			var that = this,
			thetotal;
			thetotal = parseFloat((this.parentNode.childNodes[12].innerHTML).replace(/,/g, ''));
...
$('#btnSaveOrder').click(function () {
	...
	for (k = 1; 0 < $('.new_record').length - 1; k += 1) {
		bpmdata[k - 1] = {
			...
			"order"			:  $('.new_record')[k].childNodes[3].innerHTML,
			"amount2"		:  $('.new_record')[k].childNodes[4].innerHTML,
			"currency2"		:  $('.new_record')[k].childNodes[5].innerHTML,
			"product"		:  $('.new_record')[k].childNodes[6].innerHTML,
			"beneficiary"	:  $('.new_record')[k].childNodes[7].innerHTML,
			"rate"			:  $('.new_record')[k].childNodes[8].innerHTML,
			"amount1"		:  $('.new_record')[k].childNodes[9].innerHTML,
			"currency1"		:  $('.new_record')[k].childNodes[10].innerHTML,
			"fee"			:  $('.new_record')[k].childNodes[11].innerHTML,
			"total"			:  $('.new_record')[k].childNodes[12].innerHTML,
			"benAccount"	:  $('.new_record')[k].childNodes[16].innerHTML,
			"ffc"			:  $('.new_record')[k].childNodes[17].innerHTML,
			"oldNotes"		:  $('.new_record')[k].childNodes[18].innerHTML,
			"newNotes"		:  $('.new_record')[k].childNodes[19].innerHTML,
			"notesToBendix"	:  $('.new_record')[k].childNodes[20].innerHTML
		};
		//alert(bpmdata[k-1]);
