<?php
	...
	if (isset($_GET["submit"])) {
	  ...
	  if (count($ERRORS) == 0) echo "Thank you for submitting the form!";
	}
?>
	...
	<body>
		...
		<form>
			 <p><label for="dayofyear">Favourite day of the year (1-365)</label>
				<input type="text" id="dayofyear" name="dayofyear" value="<?php if(isset($_GET['dayofyear'])) echo $_GET['dayofyear']; ?>">
				<span class="error">
					<?php if (array_key_exists("dayofyear",$ERRORS)) echo $ERRORS["dayofyear"]; ?>
				</span>
			 </p>
			 <p><label for="date">Favourite date of all time</label>
				<input type="text" id="date" name="date" value="<?php if(isset($_GET['date'])) echo $_GET['date']; ?>">
				<span class="error">
					<?php if (array_key_exists("date",$ERRORS)) echo $ERRORS["date"]; ?>
				</span>
			 </p>
			 <p><label for="word">Favourite word</label>
				<input type="text" id="word" name="word" value="<?php if(isset($_GET['word'])) echo $_GET['word']; ?>">
				<span class="error">
